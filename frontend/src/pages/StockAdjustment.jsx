...
import axios from "axios";
import { PiShippingContainer } from "react-icons/pi";
import { MdDelete, MdDeleteForever, MdEdit, MdClose } from "react-icons/md";
              ...
              {filteredstocks.length === 0 ? (
                ...
                filteredstocks.map((s, index) => (
                  <tr key={index}>
                    <td>
                      {s.warehouse_id?.store_name ||
                        warehouses.find((w) => w._id === s.warehouse_id)
                          ?.store_name ||
                        "Unknown"}
                    </td>
                    <td>{s.reason}</td>
                    ...
                    <td>
                      {s.items.map((item, idx) => (
                        <div key={idx}>
                          {item.product_id?.name || "Unknown"} — Qty:{" "}
                          {item.qty}
                        </div>
                      ))}
                    </td>
