...
exports.getWarehouses = async (req, res) => {
  try {
    let warehouses
    if(req.user.role){
      warehouses=await Warehouse.find({created_by_role:{$in:["super_admin","admin"]}}).populate("created_by","name email role")
    }else{
        warehouses = await Warehouse.find().populate("created_by","name email role")
    }
 res.json(warehouses)
  }
  catch (err) {
...
exports.addWarehouse = async (req, res) => {
  try {
    const warehouse = new Warehouse({...req.body,created_by:req.user._id,created_by_role:req.user.role})
...
exports.updateWarehouse=async (req,res) => {
  try{
    ...
    const updated=await Warehouse.findByIdAndUpdate(req.params.id,allowedFields,{new:true}).populate("created_by","name email role").populate("updated_by","name email role")
    res.json(updated)
  }
  catch(err){
    res.status(400).json({error:err.message})
